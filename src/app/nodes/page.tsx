...
export default function NodesPage() {
  ...
  return (
    <div className="animate-in fade-in duration-1000 ease-out w-full max-w-full">
      ...
      <div className="overflow-x-auto rounded-xl border border-zinc-800 bg-zinc-950 -mx-4 sm:mx-0">
        <div className="min-w-[600px] sm:min-w-0">
          <table className="w-full text-left text-sm text-zinc-400">
            <thead className="bg-zinc-900/50 text-xs uppercase text-zinc-500 border-b border-zinc-800">
              <tr>
                <th scope="col" className="px-4 sm:px-6 py-4 font-medium whitespace-nowrap">Node Name</th>
                <th scope="col" className="px-4 sm:px-6 py-4 font-medium whitespace-nowrap">CPU Usage</th>
                <th scope="col" className="px-4 sm:px-6 py-4 font-medium whitespace-nowrap">Memory</th>
                <th scope="col" className="px-4 sm:px-6 py-4 font-medium whitespace-nowrap">Latency</th>
                <th scope="col" className="px-4 sm:px-6 py-4 font-medium whitespace-nowrap">Status</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i} className="border-b border-zinc-800 last:border-0 animate-pulse">
                    <td className="px-4 sm:px-6 py-4"><div className="h-4 bg-zinc-800 rounded w-24"></div></td>
                    <td className="px-4 sm:px-6 py-4"><div className="h-4 bg-zinc-800 rounded w-12"></div></td>
                    <td className="px-4 sm:px-6 py-4"><div className="h-4 bg-zinc-800 rounded w-16"></div></td>
                    <td className="px-4 sm:px-6 py-4"><div className="h-4 bg-zinc-800 rounded w-16"></div></td>
                    <td className="px-4 sm:px-6 py-4"><div className="h-5 bg-zinc-800 rounded-full w-16"></div></td>
                  </tr>
                ))
              ) : (
                nodes.map((node) => (
                  <tr key={node.id} className="border-b border-zinc-800/50 hover:bg-zinc-900/50 transition-colors last:border-0">
                    <td className="px-4 sm:px-6 py-4 font-medium text-zinc-200 whitespace-nowrap">{node.name}</td>
